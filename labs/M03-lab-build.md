# M03 Lab: Build It with AI — The LLMOps Toolchain

## Objective
Implement a minimal ModelGateway that adds tracing, cost tagging, and a flag-driven model variant, and refactor the PA pipeline to call only the gateway.

## Prerequisites
- Completed the M03 "Understand It" lab
- Claude API access (or Claude.ai) and Python 3.10+
- The PA LLMOps sandbox from M00 (eval suite + tracing + feedback store)

## The Build (35–45 min)

### Step 1: Scaffold from the module's reference
Start from the operational-tooling pattern introduced in the module:

```python
# One seam in front of all providers. Business logic never
# imports a vendor SDK directly -> swapping tools stays cheap.
class ModelGateway:
    def __init__(self, provider, model, flags, tracer, cost_sink):
        self._p=provider; self.model=model
        self.flags=flags; self.tracer=tracer; self.cost=cost_sink
    def complete(self, prompt, *, use_case):
        model = self.flags.variant(use_case, default=self.model)  # experiments
        with self.tracer.span("model", model=model) as s:
            out = self._p.complete(prompt, model=model, temperature=0.0)
            s.set(latency_ms=out.latency_ms)
        self.cost.record(use_case, out.cost)                       # cost tagging
        return out

gw = ModelGateway(anthropic, "claude-sonnet-4-6", flags, tracer, cost_sink)
out = gw.complete(prompt, use_case="pa-determination")
```

Use Claude to adapt it to your environment:
> "Generate a starter implementation for a prior-authorization (PA) determination pipeline that accomplishes: Implement a minimal ModelGateway that adds tracing, cost tagging, and a flag-driven model variant, and refactor the PA pipeline to call only the gateway. Use Python, the PA domain (member, CPT/HCPCS, ICD-10, clinical notes, approve/deny/pend, reviewer overrides), pin model 'claude-sonnet-4-6', and return runnable, commented code."

- **Expected output:** the scaffold runs end-to-end against one sample PA request without errors.

### Step 2: Extend it to real PA cases
- Wire the scaffold to a small batch of determinations (pull 10–20 from the sandbox, spanning approve / deny / pend and at least two service types).
- Iterate with Claude on anything that breaks; ask it to explain each change.
- **Expected output:** the tool produces correct, structured results across the batch.

### Step 3: Test against the domain
- Add explicit checks tied to this module's goal (e.g., assertions, thresholds, or metrics) so a regression would fail loudly.
- Include at least one hard case (a borderline determination that drives appeals).
- **Expected output:** tests pass on good input and fail clearly on a deliberately broken case.

### Step 4: Refine for the failure modes
Harden against the anti-patterns this module calls out:
- Buying a single 'LLMOps platform' that does everything poorly and locks every layer to one vendor.
- Calling the model SDK directly everywhere — swapping providers later means touching hundreds of call sites.
- Ask Claude: *"Review this for the failure modes above and suggest the smallest changes that close them."*
- **Expected output:** a short note listing each failure mode and how your build now guards against it.

## Deliverable
A runnable artifact (script or module) that implements **the llmops toolchain** for the PA pipeline, with tests/checks and a one-paragraph README describing how it guards against the module's anti-patterns.

## Stretch Goals
- Wire the artifact into the PA CI/CD gates (M08) so it runs on every change.
- Emit its key signal to the monitoring panel (M12) and alert on a threshold breach.

## Connection to Next Module
This prepares you for **M04: Evaluation Frameworks**, which builds on what you constructed here.
