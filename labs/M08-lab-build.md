# M08 Lab: Build It with AI — CI/CD for LLM Applications

## Objective
Author a CI pipeline that runs eval, red-team, and cost gates as blocking checks and promotes a fingerprinted prompt+model+policy bundle as the deployable artifact.
...
```yaml
# .ci/pa-release.yml — prompt/model/policy changes run this
stages:
  - eval:
...
> "Generate a starter implementation for a prior-authorization (PA) determination pipeline that accomplishes: Author a CI pipeline that runs eval, red-team, and cost gates as blocking checks and promotes a fingerprinted prompt+model+policy bundle as the deployable artifact. Use Python, the PA domain (member, CPT/HCPCS, ICD-10, clinical notes, approve/deny/pend, reviewer overrides), pin model 'claude-sonnet-4-6', and return runnable, commented code."
...
## Deliverable
A runnable artifact (script or module) that implements **ci/cd for llm applications** for the PA pipeline, with tests/checks and a one-paragraph README describing how it guards against the module's anti-patterns.
...
This prepares you for **M09: Canary & Progressive Deployment**, which builds on what you constructed here.
